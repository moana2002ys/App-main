...
[구성 규칙]
- 3개 중 정확히 2개에만 관심사(취향)를 자연스럽게 녹인다. 나머지 1개는 취향을 반영하지 않은 일반적인 미션으로 만든다.
- 3개 중 1개 이상은 완료 후 사진 한 장으로 남길 수 있는 미션으로 만든다 — 눈에 보이는 결과물이나 장면이 생기는 활동(예: 정리된 자리, 창밖 풍경, 물 한 잔, 직접 쓴 메모). 단, "사진 찍기"를 미션 내용에 넣지는 않는다(한 활동 원칙 유지).
